...
from utils.utils import load_freqtrade_dir
...
@dataclass
class TradeRecord:
    # method: str                      # 使用的方法名称（如 "DistanceSelector"）
    pair: str                        # 交易对名称（如 "BTC-USDT-USDT-ETH-USDT-USDT"）
    window_start: pd.Timestamp       # 形成期窗口的起始时间
    window_end: pd.Timestamp         # 形成期窗口的结束时间
    entry_time: pd.Timestamp         # 开仓时间
    exit_time: pd.Timestamp          # 平仓时间
    side: str                        # 交易方向（如 "longA_shortB" 或 "shortA_longB"）
    beta: float                      # 配对交易的 beta 系数
    w_a: float                       # 资产A的资金权重
    w_b: float                       # 资产B的资金权重
    qty_a: float                     # 资产A的持仓数量
    qty_b: float                     # 资产B的持仓数量
    entry_price_a: float             # 资产A的开仓价格
    entry_price_b: float             # 资产B的开仓价格
    exit_price_a: float              # 资产A的平仓价格
    exit_price_b: float              # 资产B的平仓价格
    fee: float                       # 本次交易总手续费
    pnl: float                       # 本次交易的绝对盈亏（以资金单位计）
    pnl_pct: float                   # 本次交易的盈亏百分比（相对于单腿资金）
    capital_per_leg: float           # 每条腿分配的资金（通常等于 capital_per_trade）
    bars_held: int                   # 持仓的K线数量（持仓时长，单位为bar数）
...
def compute_bb(spread: pd.Series, cfg: Config):
    ma = spread.rolling(cfg.bb_window).mean()
    std = spread.rolling(cfg.bb_window).std().clip(lower=cfg.std_clip)
...
class Backtester:
    ...
    def _step_manage(self, A: pd.Series, B: pd.Series, beta: float, state: Dict,
                     start_bar: int, window_start: pd.Timestamp, window_end: pd.Timestamp):
        cfg = self.cfg
        spread = A - beta*B
        ...
        for t in range(start_bar, min(start_bar+1, len(spread))):
        ...
        while i < len(all_index):
            ...
            if pos_state is not None:
                ...
                closed_records, new_state, consumed = self._step_manage(
                    A, B, beta, pos_state, start_bar=i,
